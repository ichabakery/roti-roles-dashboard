...
-- Update handle_production_completion function with secure search path
    ...
    IF NEW.order_id IS NOT NULL THEN
      ...
      SELECT 
        ...
        format('Pesanan %s untuk %s sudah siap diambil di cabang %s', 
               ...
               (SELECT name FROM public.branches WHERE id = order_record.branch_id)),
        'success',
        order_record.id,
        'order_ready'
...
-- Update handle_order_pickup function with secure search path
...
-- Update create_custom_product function with secure search path
